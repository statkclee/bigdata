...
# R 기반 스파크-하둡 플랫폼 [^apache-spark-ubuntu] {#sparkR-hadoop}
...
<img src="fig/sparkR-arch.png" alt="R 기반 스파크-하둡 아키텍처" width="100%">
...
# 스파크-하둡 플랫폼 구축 설치과정  {#sparkR-hadoop-install}

## 부랑자(Vagrant)를 설치한다. {#sparkR-hadoop-vagrant}
...
``` {r java-shell-output, eval=FALSE}
...
``` {r java-scala-output, eval=FALSE}
$ sudo gedit .bashrc
$ . .bashrc
```
...
``` {r java-scala-install-version, eval=FALSE}
$ scala -version
...
## 하둡과 스파크 설치 [^hadoop-vs-spark] {#sparkR-hadoop-spark}
...
``` {r java-scala-shell-version, eval=FALSE}
...
``` {r java-scala-install-version-hello, eval=FALSE}
...
### 소스코드 컴파일 설치 {#sparkR-hadoop-spark-compile}

컴파일된 스파크를 사용해도 되고 소스코드를 가지고 직접 빌드해서 스파크를 설치한다.

``` {r java-scala-install-version-compile, eval=FALSE}
...
``` {r java-scala-compile-output, eval=FALSE}
parallels@ubuntu:~/spark-1.6.1$ ./bin/run-example SparkPi 10
```

``` {r java-scala-compile-output-pi, eval=FALSE}
....
...
## R, RStudio 스파크 설치 {#sparkR-hadoop-install-rstudio}

### `rJava` 설치 {#sparkR-hadoop-spark-rjava}

C++와 R을 연결시켜주는 팩키지가 `Rcpp`라면, R과 자바를 연결해주는 팩키지가 `rJava`다. 

``` {r java-scala-rjava, eval=FALSE}
...
### `SparkR` 팩키지 R 적재 {#sparkR-hadoop-spark-package}

`SparkR` 라이브러리를 스파크가 설치된 디렉토리를 지정해서 적재시킨다.

``` {r java-scala-rjava-libpath, eval=FALSE}
library(SparkR, lib.loc = "/home/vagrant/spark-1.6/R/lib/")
```

``` {r java-scala-rjava-libpath-output, eval=FALSE}
...
### `SparkR` 헬로월드 [^sparkR-rstudio] {#sparkR-hadoop-spark-hello-world}
...
``` {r java-scala-rjava-libpath-sc, eval=FALSE}
...
## 맥 운영체제 [^mac-sparkr-install] {#sparkR-hadoop-spark-mac}
...
``` {r java-scala-rjava-mac, eval=FALSE}
...
``` {r java-scala-rjava-libpath-sc-setup, eval=FALSE}
## 스파크가 설치된 환경정보를 받아온다.
