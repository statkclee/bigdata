...
# AWS를 활용하는 세가지 시나리오 {#aws-scenario}

데이터분석을 위해 AWS 활용하는 시나리오는 세가지 방법이 크게 존재한다. 

1. 시나리오 : AWS 클라우드의 저장소 기능(S3)만 활용
1. 시나리오 : AWS 클라우드의 연산기능(EC2)만 활용
1. 시나리오 : AWS 클라우드의 저장소 기능(S3)과 연산기능(EC2) 모두 활용
...
# 시나리오: AWS S3만 활용한 시나리오 {#aws-scenario-01}
...
## 데이터 분석 환경 {#aws-scenario-01-analysis}
...
## 통합개발환경(IDE) 설치 {#aws-scenario-01-analysis-ide}
...
***`Microsoft Visual C++ 10.0 is required (Unable to find vcvarsall.bat)` 오류**
...
## S3와 통합개발환경 RStudio, 쥬피터 데이터 동기화 {#aws-scenario-01-analysis-sync}
...
# 시나리오: EC2 분석 컴퓨터에서 데이터 분석 [^aws-filezilla] [^aws-filezilla-screenshot]  {#aws-scenario-02}
...
## RStudio 8787 접속 {#rstudio-connect}
...
# 시나리오: AWS 클라우드의 저장소 기능(S3)과 연산기능(EC2) 모두 활용 {#ec2-s3-s3fs-fuse}
