...
# AWS 데이터 과학 서버 [^aws-rstudio-part-1] [^aws-rstudio-part-2] {#why-data-science-server}
...
## 왜 AWS 클라우드 환경을 사용해야 하나. {#aws-cloud}
...
## EC2 인스턴스(가상 컴퓨터) 설치 {#ec2-instance}
...
### 보안 그룹(Security Group) 설정 {#ec2-security}
...
### 가상컴퓨터(인스턴스) 생성 {#ec2-virtual-instance-creation} 
...
#### 아마존 컴퓨터 이미지(Amazon Machine Images, AMIs)
...
## EC2 가상컴퓨터 로그인 {#login-ec2}

### SSH 로그인 {#login-ec2-ssh}
...
## S3 버킷 EC2 인스턴스 연결 {#connect-ec2-s3}
...
### `s3fs-fuse` 설치 [^s3fs-fuse] {#install-s3fs-fuse}
...
### RStudio 서버 설치된 EC2 인스턴스 동기화 [^rstudio-ec2-s3fs] {#s3fs-sync}
...
**`uid=1001` 오류가 나고 디렉토리 접근이 되지 않는 경우**
